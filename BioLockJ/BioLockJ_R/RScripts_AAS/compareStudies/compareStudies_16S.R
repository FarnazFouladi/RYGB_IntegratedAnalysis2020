...
fig1Pics <- file.path(outDir, paste0(taxaLevel,"_mainFigure1cd.png"))
message("Saving specific plots to individual file to compare to figure 1, see file: ", fig1Pics)
png(fig1Pics, width = 3.5, height = 7, unit="in", res=200)
theme_set(theme_classic(base_size = 9))
grid.arrange(plotList[[favs[1]]], plotList[[favs[2]]],ncol=1,nrow=2)
dev.off()

coefOut <- file.path(outDir, paste0(taxaLevel,"_coefficientsFromScatterPlots.png"))
message("Saving coefficients from scatter plots: ", coefOut)
png(coefOut, width = 5, height = 5, unit="in", res=100)
...
boxPlotFile <- file.path(outDir, paste0(taxaLevel,"_BoxPlotCorrelations.png"))
message("Saving box plots to file: ", boxPlotFile)
png(boxPlotFile, width = 5, height = 5, unit="in", res=100)
print(plot)
dev.off()
